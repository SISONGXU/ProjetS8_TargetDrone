...
while True:
	try:
		...
		file_pos = open("position.txt", "r+")
		for line in file_pos:
			values = line.split()
		x = float(values[0])
		y = float(values[1])
		surface = float(values[2])
		...
		for i in np.arange(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				if float(startX) < x < float(endX) and float(startY) < y < float(endY) and CLASSES[idx] == "person" :
					label = "{}: {:.2f}%, Person {}".format(CLASSES[idx],
						confidence * 100, i)
